...
class ForegroundService : Service() {
    ...
    companion object {
        ...
        val VESPA_DEVICE_ADDRESS = "3C:71:BF:9D:58:FE"//""24:0A:C4:13:58:EA" // <--- YOUR ESP32 MAC address here
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            if (bleManager.isConnected && intent != null) {
                ...
                if (notificationDismissed) {
                    ...
                    val successT = (bleManager as LEManager).writeNotification(buffer.substring(0, Math.min(buffer.length, 256)))
                    Timber.d("writeMessage Title {success=$successT}")

                    buffer.setLength(0)
                    buffer.append("$")
                    buffer.append(notificationBody)
                    val successB = (bleManager as LEManager).writeNotification(buffer.substring(0, Math.min(buffer.length, 256)))
                    Timber.d("writeMessage Body {success=$successB}")

                    buffer.setLength(0)
                    buffer.append("@")
                    buffer.append(notificationAppName)
                    val successA = (bleManager as LEManager).writeNotification(buffer.substring(0, Math.min(buffer.length, 256)))
                    Timber.d("writeMessage App {success=$successA}")
