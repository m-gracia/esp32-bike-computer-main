 ...
 * Re-rewrite from: https://github.com/moononournation/Arduino_GFX/blob/master/examples/ImgViewer/ImgViewerBmp/BmpClass.h
 * 
 ******************************************************************************/
#ifndef _BMPCLASS_H_
#define _BMPCLASS_H_
...
public:
        ...
        int16_t x, int16_t y, int16_t widthLimit, int16_t heightLimit)
    {
        ...
        if ((bmtype == 19778) && (bmwidth > 0) && (bmheight > 0) && (bmbpp > 0))
        {
            ...
            if (!bmpRow)
            {
                DEBUG_TFT_PRINTLN(F("bmpRow malloc failed."));
            }
            if (bmbpp < 9)
            {
                ...
                if (!bmplt)
                {
                    DEBUG_TFT_PRINTLN(F("bmplt malloc failed."));
                }
...
private:
    ...
    {               //load into globals as ints-some parameters are 32 bit, but we can't handle this size anyway
        ...
        // DEBUG_TFT_PRINTLN("bmtype: %d, bmhdrsize: %d, bmwidth: %d, bmheight: %d, bmbpp: %d\n", bmtype, bmhdrsize, bmwidth, bmheight, bmbpp);
    }
